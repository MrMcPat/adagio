...
function OtherUserProfile({ token }) {
  ...
  const [spotifyUri, setSpotifyUri] = useState(
    "spotify:track:64FzSxCxQ0cBlktqiMQBey"
  );
  ...
  useEffect(() => {
    async function handleFetch() {
      ...
      setFollowed(
        followData.data.find(
          (follow) =>
            follow.followed_user_id === filteredUser.id &&
            follow.user_id === user.data.id
        )
      );
      const emotionData = await axios.get("/emotions");
      setUserEmotions(
        emotionData.data.filter(
          (emotion) => emotion.user_id === filteredUser.id
        )
      );
      const journalData = await axios.get("/journal_entries");
      setUserJournalEntries(
        journalData.data
          .filter((entry) => entry.user_id === filteredUser.id)
          .slice(0, 3)
      );
      const postData = await axios.get("/posts");
      setUserPosts(
        postData.data
          .filter((post) => post.user_id === filteredUser.id)
          .slice(0, 3)
      );
    }
    handleFetch();
    ...
    <div style={{ textAlign: "center" }} className="user-profile-container">
      <div className="other-details">
        ...
        <div style={{ margin: "20px" }}>
          {followed ? (
            <button className="default-button" onClick={handleUnfollow}>
              Unfollow
            </button>
          ) : (
            <button className="default-button" onClick={handleFollow}>
              Follow
            </button>
          )}
        </div>
      ...
      <div className="other-music-container">
        ...
        <div className="other-music-body">
          {userProfile.favorite_songs_is_private ? (
            <h3>This user's music playlist is made private.</h3>
          ) : (
      ...
      <div className="user-recent" style={{ width: "580px" }}>
        ...
        <div className="user-recent-body" style={{ margin: "10px" }}>
          <Link to={`/user/${username}/journalentries`}>
            <button className="default-button">See All Journal Entries</button>
          </Link>
          <hr />
            ...
            userJournalEntries.map((entry) => {
              return (
                <div key={entry.id}>
                  ...
                  <br />
                  <span>
                    {entry.is_private || userProfile.journal_is_private
                      ? "Marked as private"
                      : "Public"}
                  </span>
                  <br />
                  <span>
                    {entry.created_at === entry.updated_at
                      ? `-Created on ${
                          entry.created_at.slice(0, 16).split("T")[0]
                        }, ${entry.created_at.slice(0, 16).split("T")[1]}`
                      : `-Updated on ${
                          entry.updated_at.slice(0, 16).split("T")[0]
                        }, ${entry.updated_at.slice(0, 16).split("T")[1]}`}
                  </span>
                  <span>
                    {" "}
                    ❤️{entry.heart_count} 🙏{entry.praying_count} 😮
                    {entry.shocked_count} 😞{entry.sad_count}
                  </span>
                  <hr />
                </div>
              );
      ...
      <div className="user-recent" style={{ width: "580px" }}>
        ...
        <div className="user-recent-body" style={{ margin: "10px" }}>
          <Link
            ...
            style={{ textDecoration: "none", color: "white" }}
          >
            <button className="default-button">See All Posts</button>
          </Link>
          <hr />
          {userPosts.length === 0 ? (
            <p>No posts :(</p>
